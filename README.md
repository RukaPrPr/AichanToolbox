...
<p align="center">
  <img src="https://img.shields.io/badge/version-8.1.2-7774d8?style=flat-square" alt="Version 8.1.2">
...
- 修复大批量“替换原文件”在尾段可能触发文件资源管理器崩溃的问题：先完整暂存处理结果，再通过单个 Windows Shell 事务批量回收原图；单个文件失败时仍保留原图并另存结果。
- 文件列表新增“最终画质”，显示整个工作流最终实际采用的 JPEG 画质；目标体积节点显示真实编码尝试选中的数值，不处理或未压缩显示 100。
- 修复 ZIP 解压节点文件列表无法用滚轮滚动的问题，并将滚动条移入独立内层视口，不再覆盖节点边框或触发画布缩放。
...
文件列表节点可向右自由拉宽，保留原有最小宽度与高度限制；各列仍可单独调整宽度。“最终画质”显示工作流最终实际采用的 JPEG 画质，目标体积节点显示真实命中的画质，不处理或未压缩显示 100。状态列的路径按钮保持可见，无记录或记录失效时置灰并说明原因。移动节点、调整大小、平移或缩放画布不会清除路径高亮；修改处理参数、连线或重新加载工作流后需重新预估或运行。
...
- 替换原图前先完整保存处理结果；同一批次通过单个 Windows Shell 事务回收原图，避免大批量处理时反复创建 Shell 操作。若 Windows 无法回收某个原图，会保留原图并将结果另存为带 `_processed` 后缀的文件，状态和悬停提示会说明原因；预估大小显示实际结果体积，ZIP 使用这份已保存结果。真正保存失败、取消或输出文件丢失时，不会静默回退原图打包。
...
- ZIP 解压节点的文件列表可独立使用滚轮滚动，不会缩放工作流画布。
...
- “替换原文件”会等待源文件释放，并通过专用 STA 队列交给一次 Windows Shell 批量事务移入回收站；再次运行时可以选择将上次输出采纳为新的输入基线。
